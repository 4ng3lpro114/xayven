...
export type MarketStatusAction = "activate" | "deactivate";

export type MarketActionOutcome =
  | { status: "success" }
  | { status: "error"; code: "unauthorized" | "not_found" | "generic" };

/** Same shape as requestPackageStatusChange() — dependency-injectable,
 *  testable without mocking global fetch. */
export async function requestMarketStatusChange(
  marketId: string,
  action: MarketStatusAction,
  fetchImpl: typeof fetch = fetch
): Promise<MarketActionOutcome> {
  let res: Response;
  try {
    res = await fetchImpl(`/api/admin/markets/${marketId}/status`, {
      method: "POST",
...
const ERROR_MESSAGES: Record<Exclude<MarketActionOutcome, { status: "success" }>["code"], string> = {
  unauthorized: "Tu sesión expiró — vuelve a iniciar sesión.",
  not_found: "Este mercado ya no existe.",
  generic: "No pudimos completar la acción. Intenta de nuevo.",
};

const ACTION_LABELS: Record<MarketStatusAction, string> = {
  activate: "Activar",
...
export function MarketActionButton({
  ...
  async function handleClick() {
    setLoading(true);
    setError(null);
    const outcome = await requestMarketStatusChange(marketId, action);
    setLoading(false);
