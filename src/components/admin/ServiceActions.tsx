...
export type ServiceStatusAction = "publish" | "unpublish";
 ...
 *  requestPromotionStatusChange()/requestPackageStatusChange(). */
export async function requestServiceStatusChange(
  serviceId: string,
  ...
  let res: Response;
  try {
    res = await fetchImpl(`/api/admin/services/${serviceId}/status`, {
      method: "POST",
...
const ERROR_MESSAGES: Record<Exclude<ServiceActionOutcome, { status: "success" }>["code"], string> = {
  unauthorized: "Tu sesión expiró — vuelve a iniciar sesión.",
  not_found: "Este servicio ya no existe.",
  generic: "No pudimos completar la acción. Intenta de nuevo.",
};

const ACTION_LABELS: Record<ServiceStatusAction, string> = {
  publish: "Publicar",
...
export function ServiceActionButton({
  ...
  async function handleClick() {
    setLoading(true);
    setError(null);
    const outcome = await requestServiceStatusChange(serviceId, action);
    setLoading(false);
